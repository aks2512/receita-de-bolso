...
- Execução local: rode `yarn install` (se ainda não instalado), depois `yarn test` e `yarn lint`.
- Dependências recomendadas para evitar erros de lint/testes locais (instalar como devDependencies quando necessário):
